...
class QSBK:
    ...
    def getImg(self, html):
        ...
        for imgurl in imglist:
            # print imgurl[0];
            urllib.urlretrieve(imgurl[0] + '/' + imgurl[1] + '.jpg', 'D:\qsbk\%s.jpg' % imgurl[1])
    ...
    def getPageItems(self, pageIndex):
        ...
        pattern = re.compile('<div class="author clearfix">.*?<h2>(.*?)</h2>.*?<div class="content">(.*?)</div>(.*?)' +
                             '<div class="stats">.*?<i class="number">(.*?)</i>.*?<span class="dash">.*?<i class="number">(.*?)</i>?',
                             re.S)
        ...
        for item in items:
            ...
            pageStories.append([item[0].strip(), text, item[3].strip(), item[4].strip()])
        return pageStories
